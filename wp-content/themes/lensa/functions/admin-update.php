...
function colabsthemes_framework_update_page_set(){
        ...
        elseif(isset($_POST['colabs_ftp_cred'])){
             ...
             $cred = unserialize($_POST['colabs_ftp_cred']);
             $filesystem = WP_Filesystem($cred);
...
function colabsthemes_themes_update_page_set(){
	...
	$cookie_content = $wp_filesystem->get_contents( $cookiefile );
	...
	if ($check_cookie!=true){
		...
		<form method="post"  enctype="multipart/form-data" id="colabsform" name="login" class="colabs-login-form">
			...
			<input type="submit" name="colabs_theme_login" value="<?php _e('Log In','colabsthemes');?>" class="button" />
...
function colabsthemes_framework_update_check(){
	...
	$data['remoteversion'] = colabs_get_fw_version('http://colorlabsproject.com/updates/backbone/functions-changelog.txt');
	}else{
...
function colabsthemes_framework_update_head(){
  ...
  if(isset($_REQUEST['page'])){
	...
	if( 'colabsthemes_framework_update' == $_page){
		...
		$theme = wp_get_theme();
		if('backbone'==$theme->Template){
		$temp_file_addr = download_url( 'http://colorlabsproject.com/updates/backbone/framework.zip' );
		}else{
...
function colabs_theme_update(){
		...
		if ($check_cookie==true){
			...
			if ( is_wp_error($do_unzip) ) {
				...
				//The source file was not found or is invalid
					...
					echo "<div id='colabs-no-archive-warning' class='updated fade' ><p>"; 
					echo sprintf(__("This account is ineligible for the update. Please <a href='http://colorlabsproject.com/member/signup' target='_blank'>renew your subscription</a> or <a href='%s'>sign in</a> with a different account.","colabsthemes"),$signin);
					echo "</p></div>";
				}
				if('empty_archive' == $error) {
						echo "<div id='colabs-empty-archive-warning' class='updated fade' ><p>". __("Failed: Empty Archive","colabsthemes")."</p></div>";
	...
	if($theme_backup=='true'){
		...
		$to = $wp_filesystem->wp_content_dir() . "themes/";
		$file = $to.'backup_'.strtolower($theme_name).'_'.time().'.zip';
		$dirName = $to.strtolower($theme_name);
		zipped_file($file,$dirName);
