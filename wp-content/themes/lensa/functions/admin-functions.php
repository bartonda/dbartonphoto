...
if ( !function_exists( 'vt_resize') ) {
	function vt_resize( $attach_id = null, $img_url = null, $width, $height, $crop = false ) {
		...
		if ( $image_src[1] > $width AND $image_src[2] > $height ) {
			...
			$new_img_path = colabs_image_resize( $file_path, $width, $height, $crop );
...
if ( ! function_exists( 'colabs_feed_link' ) ) { 
	function colabs_feed_link ( $output, $feed = null ) {	
		...
		if ( isset($colabs_options[ 'colabs_feedlinkurl' ]) && ( $feed == $default ) && ( ! stristr( $output, 'comments' ) ) ) $output = $colabs_options[ 'colabs_feedlinkurl' ];	
		return $output;	
 ...
 */
function colabs_image_resize( $file, $max_w, $max_h, $crop = false, $suffix = null, $dest_path = null, $jpeg_quality = 90 ) {
	
	        $image = colabs_load_image( $file );
 ...
 */
function colabs_load_image( $file ) {
	        if ( is_numeric( $file ) )
